...
(function(window, angular, undefined) {
	...
	angular.module('satellizer', [])
		...
		.provider('$auth', ['satellizer.config', function(config) {
			...
			this.opauth = function(params) {
				oauth(params);
				config.providers[params.name].type = 'opauth';
			};
		...
		.factory('satellizer.oauth', [
			...
			'satellizer.Oauth2',
			'satellizer.Opauth',
			function($q, $http, config, shared, Oauth1, Oauth2, Opauth) {
				var oauth = {};

				oauth.authenticate = function(name, isLinking, userData) {

					switch (config.providers[name].type) {
						case '1.0':
							var provider = new Oauth1();
							break;
						case '2.0':
							var provider = new Oauth2();
							break;
						case 'opauth':
							var provider = new Opauth();
							break;
					}
		...
		.factory('satellizer.Opauth', [
			...
			function($q, $http, popup, utils, config) {
				return function() {
					...
					opauth.open = function(options, userData) {
						angular.extend(defaults, options);

						return popup.open(defaults.authorizationEndpoint, defaults.popupOptions)
							.then(function(oauthData) {
								return opauth.exchangeForToken(oauthData, userData);
							});

					};

					opauth.exchangeForToken = function(oauthData, userData) {
